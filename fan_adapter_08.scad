...
//
// fan_adapter_08.scad
// Final improved adapter with upward-sloping cable channel
...
module fan_adapter(
    ...
    top_flange_thk      = 5,     
    adapter_height      = 45,    // Height of the main adapter body
    offset_x            = 0,     // Offset to accommodate wall
    offset_y            = 0,     
    bottom_fan_size     = 120,   // 120mm PC fan size
    bottom_flange_thk   = 8,     
    wall_thickness      = 4,     
    ...
    cable_entry_height  = 25,    // Height from bottom for cable entry
    cable_exit_height   = 45     // Height for cable exit near top
) {
    ...
    // 
    // 6) Cable Pass-Through: Upward-sloping channel
    //    Creates a path from bottom/outside to top/inside with minimal airflow disruption
    //
    module cable_split_channel() {
        ...
        // Entry point (outside wall)
        entry_x = outer_radius * dir_x;
        entry_y = outer_radius * dir_y;
        
        // Exit point (inside the adapter near the top hole)
        exit_x = offset_x + (top_hole_d/2 * 0.7) * dir_x;
        exit_y = offset_y + (top_hole_d/2 * 0.7) * dir_y;
        
        // Create channel with aerodynamic shaping and smooth entry
        hull() {
            // Entry point at outside wall
            ...
            mid_z = (cable_entry_height + cable_exit_height) / 2;
            translate([mid_x, mid_y, mid_z])
                rotate([0, 45, cable_channel_angle])
                    scale([1, 1, 0.7])
                        cylinder(d = cable_channel_width, h = 0.1, center = true);
            
            // Exit point near the top (inside the adapter)
            translate([exit_x, exit_y, cable_exit_height])
                scale([1, 1, 0.6]) 
                    sphere(d = cable_channel_width);
        }
        
        // Ensure clean entry through the wall
        translate([entry_x, entry_y, cable_entry_height])
            rotate([0, 90, cable_channel_angle])
                scale([1.2, 1.2, 1])
                    cylinder(d = cable_channel_width, h = wall_thickness * 1.5, center = true);
        
        // Add a smooth transition to the inner airflow space at the exit
        hull() {
            translate([exit_x, exit_y, cable_exit_height])
                sphere(d = cable_channel_width * 0.8);
            
            translate([exit_x * 0.8, exit_y * 0.8, cable_exit_height * 0.95])
                sphere(d = cable_channel_width * 0.7);
            
            translate([offset_x, offset_y, cable_exit_height * 0.9])
...
fan_adapter(
    ...
    offset_y = 0,
    adapter_height = 45,       // Height of the adapter body
    ...
    cable_entry_height = 25,   // Entry point for cable at the wall
    cable_exit_height = 45     // Exit point at top of adapter (into airflow path)
);